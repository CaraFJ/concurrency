...
is not allowed.
        ...
        # raise ValueError(msg)
        warnings.warn(
            msg[:-1] + " and will raise exceptions in a future release.", DeprecationWarning, stacklevel=4,
        )
...
class EntryPoint:
    ...
    def load(self, require=True, *args, **kwargs):
        ...
        if not require or args or kwargs:
            warnings.warn(
                "Parameters to load are deprecated.  Call .resolve and " ".require separately.", PkgResourcesDeprecationWarning, stacklevel=2,
            )
        if require:
    ...
    pattern = re.compile(r"\s*" r"(?P<name>.+?)\s*" r"=\s*" r"(?P<module>[\w.]+)\s*" r"(:\s*(?P<attr>[\w.]+))?\s*" r"(?P<extras>\[.*\])?\s*$")
