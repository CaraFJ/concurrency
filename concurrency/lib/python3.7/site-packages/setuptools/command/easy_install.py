...
class easy_install(Command):
    ...
    boolean_options = ["zip-ok", "multi-version", "exclude-scripts", "upgrade", "always-copy", "editable", "no-deps", "local-snapshots-ok", "version"]
    ...
    def install_egg(self, egg_path, tmpdir):
        ...
        if not samefile(egg_path, destination):
            ...
            try:
                ...
                self.execute(
                    f, (egg_path, destination), (m + " %s to %s") % (os.path.basename(egg_path), os.path.dirname(destination)),
                )
                update_dist_caches(
    ...
    def install_wheel(self, wheel_path, tmpdir):
        ...
        try:
            self.execute(
                wheel.install_as_egg, (destination,), ("Installing %s to %s") % (os.path.basename(wheel_path), os.path.dirname(destination)),
            )
        finally:
            update_dist_caches(destination, fix_zipimporter_caches=False)
        self.add_output(destination)
