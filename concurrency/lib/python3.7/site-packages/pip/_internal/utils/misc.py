...
if MYPY_CHECK_RUNNING:
    from typing import Optional, Tuple, Iterable, List, Match, Union, Any, Mapping, Text, AnyStr, Container  # noqa: F401
        ...
        for member in tar.getmembers():
            ...
            if member.isdir():
                ...
                try:
                    ...
                    logger.warning(
                        "In the tar file %s the member %s is invalid: %s", filename, member.name, exc,
                    )
                    continue
            else:
                try:
...
def unpack_file(
    ...
    if content_type == "application/zip" or filename.lower().endswith(ZIP_EXTENSIONS) or zipfile.is_zipfile(filename):
        ...
        logger.critical(
            "Cannot unpack file %s (downloaded from %s, content-type: %s); " "cannot detect archive format", filename, location, content_type,
        )
...
def redact_netloc(netloc):
    ...
    return "{user}{password}@{netloc}".format(user=urllib_parse.quote(user), password=password, netloc=netloc)
