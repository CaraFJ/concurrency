...
def check_install_build_global(options, check_options=None):
    ...
    if any(map(getname, names)):
        ...
        warnings.warn(
            "Disabling all use of wheels due to the use of --build-options " "/ --global-options / --install-options.", stacklevel=2,
        )
...
help_ = partial(Option, "-h", "--help", dest="help", action="help", help="Show help.",)  # type: Callable[..., Option]
...
verbose = partial(
    Option, "-v", "--verbose", dest="verbose", action="count", default=0, help="Give more output. Option is additive, and can be used up to 3 times."
)  # type: Callable[..., Option]

no_color = partial(
    Option, "--no-color", dest="no_color", action="store_true", default=False, help="Suppress colored output",
)  # type: Callable[..., Option]

version = partial(Option, "-V", "--version", dest="version", action="store_true", help="Show version and exit.",)  # type: Callable[..., Option]
...
log = partial(
    Option, "--log", "--log-file", "--local-log", dest="log", metavar="path", help="Path to a verbose appending log."
)  # type: Callable[..., Option]
...
proxy = partial(
    Option, "--proxy", dest="proxy", type="str", default="", help="Specify a proxy in the form [user:passwd@]proxy.server:port."
)  # type: Callable[..., Option]
...
cert = partial(Option, "--cert", dest="cert", type="str", metavar="path", help="Path to alternate CA bundle.",)  # type: Callable[..., Option]
...
def prefer_binary():
    # type: () -> Option
    return Option(
        "--prefer-binary", dest="prefer_binary", action="store_true", default=False, help="Prefer older binary packages over newer source packages."
    )


cache_dir = partial(
    Option, "--cache-dir", dest="cache_dir", default=USER_CACHE_DIR, metavar="dir", help="Store the cache data in <dir>."
)  # type: Callable[..., Option]
...
no_cache = partial(
    Option, "--no-cache-dir", dest="cache_dir", action="callback", callback=no_cache_dir_callback, help="Disable the cache.",
)  # type: Callable[..., Option]
...
ignore_requires_python = partial(
    Option, "--ignore-requires-python", dest="ignore_requires_python", action="store_true", help="Ignore the Requires-Python information."
)  # type: Callable[..., Option]
...
no_use_pep517 = partial(
    Option, "--no-use-pep517", dest="use_pep517", action="callback", callback=no_use_pep517_callback, default=None, help=SUPPRESS_HELP
)  # type: Any
...
no_clean = partial(Option, "--no-clean", action="store_true", default=False, help="Don't clean up build directories.")  # type: Callable[..., Option]
...
always_unzip = partial(Option, "-Z", "--always-unzip", dest="always_unzip", action="store_true", help=SUPPRESS_HELP,)  # type: Callable[..., Option]
...
index_group = {"name": "Package Index Options", "options": [index_url, extra_index_url, no_index, find_links,]}  # type: Dict[str, Any]
