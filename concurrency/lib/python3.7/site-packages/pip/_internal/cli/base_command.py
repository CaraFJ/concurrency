...
class Command(object):
    ...
    def _build_session(self, options, retries=None, timeout=None):
        ...
        session = PipSession(
            cache=(normalize_path(os.path.join(options.cache_dir, "http")) if options.cache_dir else None),
...
class RequirementCommand(Command):
        ...
        if not (args or options.editables or options.requirements):
            ...
            if options.find_links:
                raise CommandError(
                    ...
                    '(maybe you meant "pip %(name)s %(links)s"?)' % dict(opts, links=" ".join(options.find_links))
                )
            else:
    ...
    def _build_package_finder(
        ...
        if options.no_index:
            logger.debug(
                "Ignoring indexes: %s", ",".join(redact_password_from_url(url) for url in index_urls),
            )
            index_urls = []
