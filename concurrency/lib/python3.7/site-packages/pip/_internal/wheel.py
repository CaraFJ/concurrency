...
if MYPY_CHECK_RUNNING:
    from typing import Dict, List, Optional, Sequence, Mapping, Tuple, IO, Text, Any, Union, Iterable  # noqa: F401
...
"""
    ...
    with open_for_csv(record, "r") as record_in:
        with open_for_csv(temp_record, "w+") as record_out:
            ...
            outrows = get_csv_rows_for_installed(reader, installed=installed, changed=changed, generated=generated, lib_dir=lib_dir,)
...
class WheelBuilder(object):
    ...
    def _base_setup_args(self, req):
        ...
        # virtualenv.
        return [sys.executable, "-u", "-c", SETUPTOOLS_SHIM % req.setup_py] + list(self.global_options)
    ...
    def _build_one_pep517(self, req, tempd, python_tag=None):
        ...
        try:
            ...
            wheel_name = req.pep517_backend.build_wheel(tempd, metadata_directory=req.metadata_directory)
            if python_tag:
    ...
    def _build_one_legacy(self, req, tempd, python_tag=None):
        ...
        with open_spinner(spin_message) as spinner:
            ...
            try:
                output = call_subprocess(wheel_args, cwd=req.setup_py_dir, show_stdout=False, spinner=spinner)
            except Exception:
                spinner.finish("error")
                logger.error("Failed building wheel for %s", req.name)
                return None
            names = os.listdir(tempd)
            wheel_path = get_legacy_build_wheel_path(names=names, temp_dir=tempd, req=req, command_args=wheel_args, command_output=output,)
            return wheel_path
    ...
    def build(
        ...
        # Build the wheels.
        logger.info(
            "Building wheels for collected packages: %s", ", ".join([req.name for (req, _) in buildset]),
        )
