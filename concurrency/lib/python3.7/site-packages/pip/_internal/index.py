...
if MYPY_CHECK_RUNNING:
    ...
    from typing import Tuple, Optional, Any, List, Union, Callable, Set, Sequence, Iterable, MutableMapping  # noqa: F401
...
def _get_html_page(link, session=None):
    ...
    try:
        ...
        logger.debug(
            "Skipping page %s because it looks like an archive, and cannot " "be checked by HEAD.", link,
        )
    except _NotHTML as exc:
        logger.debug(
            "Skipping page %s because the %s request got Content-Type: %s", link, exc.request_desc, exc.content_type,
        )
...
class PackageFinder(object):
    ...
    def __init__(
        ...
        self.secure_origins = [("*", host, "*") for host in (trusted_hosts if trusted_hosts else [])]  # type: List[SecureOrigin]
        ...
        self.valid_tags = get_supported(versions=versions, platform=platform, abi=abi, impl=implementation,)
    ...
    def _sort_locations(locations, expand_dir=False):
        ...
        for url in locations:
            ...
            if is_local_path or is_file_url:
                ...
                logger.warning(
                    "Url '%s' is ignored. It is either a non-existing " "path or lacks a specific scheme.", url,
                )
    ...
    def _validate_secure_origin(self, logger, location):
        ...
        for secure_origin in SECURE_ORIGINS + self.secure_origins:
            ...
            try:
                ...
                if origin[1] and origin[1].lower() != secure_origin[1].lower() and secure_origin[1] != "*":
                    continue
            else:
    ...
    def find_all_candidates(self, project_name):
        ...
        url_locations = [
            link
            for link in itertools.chain((Link(url) for url in index_url_loc), (Link(url) for url in fl_url_loc),)
    ...
    def find_requirement(self, req, upgrade):
        ...
        if not upgrade and installed_version is not None:
            if best_installed:
                logger.debug(
                    "Existing installed version (%s) is most up-to-date and " "satisfies requirement", installed_version,
                )
            else:
                logger.debug(
        ...
        logger.debug("Using version %s (newest of versions: %s)", best_candidate.version, ", ".join(sorted(compatible_versions, key=parse_version)))
    ...
    def _link_package_versions(self, link, search):
        ...
        try:
            ...
            logger.debug("Package %s has an invalid Requires-Python entry: %s", link.filename, link.requires_python)
            support_this_python = True

        if not support_this_python:
            logger.debug(
                "The package %s is incompatible with the python " "version in use. Acceptable python versions are: %s", link, link.requires_python
            )
            return None
