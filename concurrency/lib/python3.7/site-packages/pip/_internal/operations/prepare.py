...
class RequirementPreparer(object):
    ...
    def prepare_linked_requirement(
        ...
        with indent_log():
            ...
            try:
                ...
                unpack_url(
                    req.link, req.source_dir, download_dir, autodelete_unpacked, session=session, hashes=hashes, progress_bar=self.progress_bar
                )
            except requests.HTTPError as exc:
                logger.critical(
                    "Could not install requirement %s because of error %s", req, exc,
                )
                raise InstallationError("Could not install requirement %s because of HTTP " "error %s for URL %s" % (req, exc, req.link))
    ...
    def prepare_editable_requirement(
        ...
        with indent_log():
            if require_hashes:
                raise InstallationError(
                    "The editable requirement %s cannot be installed when " "requiring hashes, because there is no single file to " "hash." % req
                )
    ...
    def prepare_installed_requirement(self, req, require_hashes, skip_reason):
        ...
        assert skip_reason is not None, "did not get skip reason skipped but req.satisfied_by " "is set to %r" % (req.satisfied_by,)
