...
class InstallRequirement(object):
    ...
    def _correct_build_location(self):
        ...
        logger.debug(
            "Moving package %s from %s to new location %s", self, display_path(old_location), display_path(new_location),
        )
            ...
            if self.editable:
                ...
                for root, dirs, files in os.walk(base):
                    ...
                    for dir in list(dirs):
                        ...
                        # environment
                        if os.path.lexists(os.path.join(root, dir, "bin", "python")) or os.path.exists(
    ...
    def assert_source_matches_version(self):
        ...
        if self.req.specifier and version not in self.req.specifier:
            ...
            logger.debug(
                "Source in %s has version %s, which satisfies requirement %s", display_path(self.source_dir), version, self,
            )
    ...
    def install_editable(
        ...
        with indent_log():
            ...
            with self.build_env:
                call_subprocess(
                    [sys.executable, "-c", SETUPTOOLS_SHIM % self.setup_py] + list(global_options) + ["develop", "--no-deps"] + list(install_options),
    ...
    def archive(self, build_dir):
        ...
        if os.path.exists(archive_path):
            response = ask_path_exists("The file %s exists. (i)gnore, (w)ipe, (b)ackup, (a)bort " % display_path(archive_path), ("i", "w", "b", "a"))
            if response == "i":
        ...
        with TempDirectory(kind="record") as temp_dir:
            ...
            with open_spinner(msg) as spinner:
                with indent_log():
                    with self.build_env:
                        call_subprocess(
                            install_args + install_options, cwd=self.setup_py_dir, show_stdout=False, spinner=spinner,
                        )
