...
class HTTPSConnection(HTTPConnection):
    ...
    def connect(self):
        ...
        self.sock = ssl_wrap_socket(
            sock=conn, keyfile=self.key_file, certfile=self.cert_file, ssl_context=self.ssl_context, server_hostname=self.server_hostname
        )
...
class VerifiedHTTPSConnection(HTTPSConnection):
    ...
    def connect(self):
        ...
        if is_time_off:
            warnings.warn(
                ("System time is way off (before {0}). This will probably " "lead to SSL verification errors").format(RECENT_DATE), SystemTimeWarning
            )
