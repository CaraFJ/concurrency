...
def ip_address(address):
    ...
    if isinstance(address, bytes):
        raise AddressValueError(
            "%r does not appear to be an IPv4 or IPv6 address. " "Did you pass in a bytes (str in Python 2) instead of" " a unicode object?" % address
        )
...
def ip_network(address, strict=True):
    ...
    if isinstance(address, bytes):
        raise AddressValueError(
            "%r does not appear to be an IPv4 or IPv6 network. " "Did you pass in a bytes (str in Python 2) instead of" " a unicode object?" % address
        )
...
class _IPAddressBase(_TotalOrderingMixin):
    ...
    def _check_packed_address(self, address, expected_len):
        ...
        if address_len != expected_len:
            msg = (
                "%r (len %d != %d) is not permitted as an IPv%d address. " "Did you pass in a bytes (str in Python 2) instead of" " a unicode object?"
            )
...
class IPv4Network(_BaseV4, _BaseNetwork):
    ...
    def is_global(self):
        ...
        """
        return (
            not (self.network_address in IPv4Network("100.64.0.0/10") and self.broadcast_address in IPv4Network("100.64.0.0/10"))
